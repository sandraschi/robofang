"""
openfang/supervisor.py
======================
Lightweight process supervisor for the OpenFang bridge.
...
# Command to start the bridge — same as running `python -m openfang.main`
# We use sys.executable so we always get the same Python that ran the supervisor.
_REPO_ROOT = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
BRIDGE_CMD = [sys.executable, "-m", "openfang.main"]
...
logger = logging.getLogger("openfang_supervisor")
...
app = FastAPI(title="OpenFang Supervisor", version="1.0.0")
...
if __name__ == "__main__":
    logger.info(f"OpenFang Supervisor starting on port {SUPERVISOR_PORT}")
