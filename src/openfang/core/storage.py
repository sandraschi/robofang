"""
OpenFang Storage Layer: Persistent state management using SQLite.
...
class OpenFangStorage:
    """
    Sovereign storage engine for OpenFang.
    Ensures that fleet state survives reboots.
    """

    def __init__(self, db_path: Optional[Path] = None):
        if not db_path:
            # Default to repo_root/data/openfang.db
            repo_root = Path(__file__).parent.parent.parent.parent
            data_dir = repo_root / "data"
            data_dir.mkdir(exist_ok=True)
            db_path = data_dir / "openfang.db"

        self.db_path = db_path
        self.logger = logging.getLogger("openfang.core.storage")
        self._init_db()
    ...
    def _init_db(self):
        ...
        self.logger.info(f"Initializing OpenFang Storage at {self.db_path}")
