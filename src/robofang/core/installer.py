...
class HandInstaller:
    ...
    def install(self, hand_id: str) -> Dict[str, Any]:
        ...
        if target_dir.exists():
            return {"success": True, "message": f"Hand '{hand_id}' already installed at {target_dir}."}
            ...
            if item.install_script:
                ...
                if script_path.exists():
                    ...
                    if script_path.suffix == ".ps1":
                        subprocess.run(["pwsh", "-File", str(script_path)], cwd=str(target_dir), check=True)
                    else:
                        subprocess.run([str(script_path)], cwd=str(target_dir), check=True, shell=True)
