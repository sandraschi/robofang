...
import { X, ExternalLink, Bot, Cpu, Activity, Info } from 'lucide-react';
...
function StarField() {
    ...
    const sphere = useMemo(() => {
        ...
        for (let i = 0; i < 5000; i++) {
            const stride = i * 3;
            const r = 50;
            const theta = 2 * Math.PI * (i / 5000) * 123.456;
...
function FleetNode({ name, position, color, onClick }: NodeProps) {
    ...
    const shortName = (name || 'unknown').split('.')[0].substring(0, 15);
    ...
    return (
        <div className="w-full h-[550px] bg-[#050512] rounded-3xl overflow-hidden border border-white/5 shadow-2xl relative group">
            ...
            <Canvas gl={{ antialias: true, alpha: true }} dpr={[1, 2]}>
                <color attach="background" args={['#050512' as any]} />
            ...
            <div className="absolute bottom-8 left-8 z-10 grid grid-cols-2 md:grid-cols-6 gap-2 opacity-80 hover:opacity-100 transition-opacity">
               <HubLink to="/" label="Dashboard" icon="🏠" color="from-blue-500/20 to-indigo-500/20" />
               <HubLink to="/creative" label="Creative" icon="🎨" color="from-purple-500/20 to-pink-500/20" />
               <HubLink to="/knowledge" label="Knowledge" icon="🧠" color="from-amber-500/20 to-yellow-500/20" />
